...
namespace common\components\mongodb\operation;
...
use common\components\mongodb\Exception\UnexpectedValueException;
use ArrayIterator;
use stdClass;
use Traversable;
...
class Aggregate implements Executable
{
    ...
    {
        $isCursorSupported = \common\components\mongodb\server_supports_feature($server, self::$wireVersionForCursor);
    ...
    private function createCommand(Server $server, $isCursorSupported)
    {
        ...
        if (isset($this->options['bypassDocumentValidation']) && \common\components\mongodb\server_supports_feature($server, self::$wireVersionForDocumentLevelValidation)) {
        ...
        if (isset($this->options['readConcern']) && \common\components\mongodb\server_supports_feature($server, self::$wireVersionForReadConcern)) {
            $cmd['readConcern'] = \common\components\mongodb\read_concern_as_document($this->options['readConcern']);
        }
